/* ============================================================
   app.js v33 — three sections only:
...
function renderPlan(p) {
  ...
  let lastLvl = null, num = 0;
  const hasBreak = (p.entries || []).some((e) => e.branch === 'break');
  const ehTitle = hasBreak ? '🎯 จุดเข้าตามทฤษฎี OI · ทุกด่าน = จุดตัดสินใจ (reject / break เกิดได้ทางเดียว)' : '🎯 จุดเข้าตามทฤษฎี OI · วันนี้เฉพาะ reject ตามเทรนด์ (กฎทอง)';
  const entries = (p.entries && p.entries.length)
    ? `<div class="plan-entries"><div class="plan-eh">${ehTitle}</div>${p.entries_intro ? `<div class="entry-intro">${esc(p.entries_intro)}</div>` : ''}${p.entries.map((en) => {
        const tp = en.tp || [];
        let hdr = '';
        ...
        const roleTagTxt = (en.role === 'counter' && en.branch === 'break') ? '<span class="role-tag counter">พลิกแผน · ไม้ครึ่ง</span>'
          : (en.role === 'counter' && en.size !== 'half') ? '<span class="role-tag counter">สวน bias · ไม้ปกติ</span>'
          : (en.role === 'main' && en.size === 'half') ? '<span class="role-tag main">ตามเทรนด์ · ไม้ครึ่ง' + (en.flag === 'rr' ? ' (⚠RR)' : '') + '</span>'
          : (en.role === 'range' && en.size === 'half') ? '<span class="role-tag range">ในกรอบ · ไม้ครึ่ง' + (en.flag === 'rr' ? ' (⚠RR)' : '') + '</span>' : roleTag(en.role);
